...
<script>
    export default {
      ...
      beforeCreate:function(){
        this.$http.get('http://127.0.0.1:3000/essay/find').then((result)=>{
      ...
      methods:{
        ...
        updsubmit:function(){
          this.$http.get('http://127.0.0.1:3000/essay/update?id='+this.id+"&title="+this.newtitle+"&content="+this.newcontent).then(result=>{
        ...
        addsubmit:function(){
          this.$http.get('http://127.0.0.1:3000/essay/insert?title='+this.newtitle+"&content="+this.newcontent).then(result=>{
            if(result.body.code==1){
              alert('添加成功!');
              this.$router.replace('/blank1')
            }
        ...
        deletes:function(e){
          if(confirm("确定删除?")){
          this.id=e.target.parentNode.parentNode.firstChild.innerHTML;
           this.$http.get('http://127.0.0.1:3000/essay/delete?id='+this.id).then(result=>{
             if(result.body.code==1){
               alert('删除成功!');
               this.$router.replace('/blank1')
            }
